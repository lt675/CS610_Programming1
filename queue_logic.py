...
class CounterQueue:
    ...
    def enqueue(self, counter_id, customer):
        if counter_id in self.counters:
            try:
                self.counters[counter_id].put_nowait(customer)
